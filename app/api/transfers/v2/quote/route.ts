  ...
  try {
    ...
    if (!originId || !destinationId || originId === destinationId) {
      return NextResponse.json(
        { error: "Debes enviar origen y destino distintos." },
        { status: 400 }
      );
    }
    ...
    for (const priceEntry of route.prices) {
      ...
      const finalPrice =
        overrideForVehicle.find(
          (override) =>
            override.originLocationId === origin.id && override.destinationLocationId === destination.id
        )?.price ??
        overrideForVehicle.find(
          (override) => override.originLocationId === origin.id && !override.destinationLocationId
        )?.price ??
        overrideForVehicle.find(
    ...
    if (!vehicles.length) {
      return NextResponse.json(
        { error: "No hay vehículos disponibles para ese número de pasajeros." },
        { status: 404 }
      );
    }
