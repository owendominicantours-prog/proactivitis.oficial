...
export async function createSupplierOfferAction(formData: FormData) {
  ...
  const title = String(formData.get("title") ?? "").trim() || "Oferta especial";
  ...
  if (discountType === "PERCENT" && discountValue > 95) {
    throw new Error("El porcentaje maximo permitido es 95%.");
  }
  ...
  if (validTourIds.length === 0) {
    throw new Error("No tienes permiso para crear ofertas con esos tours.");
  }
