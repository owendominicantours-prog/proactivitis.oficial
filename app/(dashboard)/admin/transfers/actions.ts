...
export async function importTransferLocationsAction(formData: FormData) {
  const csvField = formData.get("csvFile");
  const fallbackZoneId = formData.get("zoneId");

  if (!csvField || !(csvField instanceof File)) {
    throw new Error("Envía un archivo CSV válido.");
  }
  ...
  if (!rows.length) {
    throw new Error("Tu CSV no contiene filas válidas.");
  }
  ...
  if (nameIndex === -1 || slugIndex === -1 || typeIndex === -1) {
    throw new Error("Las columnas mínimas deben ser: name, slug, type.");
  }

  const zones = await prisma.transferZoneV2.findMany();
  const zoneSlugMap = new Map(zones.map((zone) => [zone.slug.toLowerCase(), zone]));
  const fallbackZone = fallbackZoneId && typeof fallbackZoneId === "string" ? await prisma.transferZoneV2.findUnique({ where: { id: fallbackZoneId } }) : null;
  ...
  for (const row of parsed) {
    ...
    if (zoneSlugIndex !== -1) {
      const zoneSlugValue = cells[zoneSlugIndex]?.toLowerCase();
      if (zoneSlugValue) {
        zone = zoneSlugMap.get(zoneSlugValue) ?? zone;
      }
    }

    if (!zone) {
      throw new Error(`No se encontró la zona para la fila con slug "${slug}".`);
    }

    const normalizedType = typeValue.toUpperCase();
    const type = Object.values(TransferLocationType).includes(normalizedType as TransferLocationType)
      ? (normalizedType as TransferLocationType)
