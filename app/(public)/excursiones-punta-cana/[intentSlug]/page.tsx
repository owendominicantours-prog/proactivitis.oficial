...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonical = `${NUEVA_GENERACION_BASE_URL}${buildNuevaGeneracionIntentPath(intent.slug)}`;
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: tours.map((tour, index) => ({
          ...
          position: index + 1,
          name: tour.title,
          url: `${NUEVA_GENERACION_BASE_URL}${buildNuevaGeneracionIntentTourPath(tour.slug, intent.slug)}`
        }))
      },
      {
        "@type": "BreadcrumbList",
        ...
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "Excursiones Punta Cana", item: `${NUEVA_GENERACION_BASE_URL}${NUEVA_GENERACION_HUB_PATH}` },
    ...
    <main className="bg-white text-slate-950">
      ...
      <section className="border-b border-slate-200 bg-slate-950 text-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-14 sm:px-8 lg:grid-cols-[1fr_360px] lg:px-12">
          <div>
            <Link href={NUEVA_GENERACION_HUB_PATH} className="text-xs font-black uppercase tracking-[0.3em] text-emerald-300">
