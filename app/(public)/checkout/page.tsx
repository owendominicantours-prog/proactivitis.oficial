...
const paymentMethods = [
  { id: 'card', label: 'Tarjeta de crédito', icons: ['Visa', 'Mastercard'] },
...
export default function CheckoutPage() {
  ...
  const [language, setLanguage] = useState('Español / Inglés');
      ...
      <div className='space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-xl'>
        ...
        <div className='space-y-3 text-sm text-slate-600'>
          <div className='flex items-center justify-between'>
            ...
            <strong>
              {recommendedReservation.adults} adults · {recommendedReservation.children} niños
            </strong>
          </div>
          <div className='flex items-center justify-between'>
            <span className='flex items-center gap-2'>
  ...
  const handlePaymentSubmit = (event: FormEvent) => {
    ...
    const newErrors: Record<string, string> = {};
    if (!paymentType) newErrors.paymentType = 'Selecciona un método';
    if (paymentType === 'card' && cardNumber.replace(/\s+/g, '').length < 16)
      newErrors.cardNumber = 'Ingresa un número válido';
    setErrors(newErrors);
    if (Object.keys(newErrors).length === 0) {
      alert('Tu reserva ha sido procesada con éxito.');
    }
  };
                      ...
                      <div className='space-y-5 pt-6'>
                        {stepIdx === 0 && (
                          <div className='space-y-4'>
                            {[
                              ...
                              { label: 'Apellido', key: 'lastName', placeholder: 'Martínez' },
                              { label: 'Correo electrónico', key: 'email', placeholder: 'tucorreo@proactivitis.com' },
                              { label: 'Confirmar correo', key: 'confirmEmail', placeholder: 'Confirma tu correo' },
                              { label: 'Teléfono', key: 'phone', placeholder: '+1 809 555 555' }
                            ].map((field) => (
                          ...
                          <div className='space-y-4'>
                            ...
                            <div>
                              ...
                              >
                                ...
                                <option>Español / Francés</option>
                                <option>Inglés / Portugués</option>
                              </select>
                            </div>
                            <div className='space-y-2'>
                        ...
                        {stepIdx === 2 && (
                          <form onSubmit={handlePaymentSubmit} className='space-y-4'>
                            ...
                            {paymentType === 'card' && (
                              <div className='space-y-2'>
                                <label className='text-xs uppercase tracking-[0.3em] text-slate-500'>Número de tarjeta</label>
                                <input
                                  id='cardNumber'
                                  value={cardNumber}
                            ...
                            <div className='space-y-2'>
                              <label className='text-xs uppercase tracking-[0.3em] text-slate-500'>País de facturación</label>
                            ...
                            <div className='space-y-2'>
                              <p className='text-[11px] uppercase tracking-[0.4em] text-slate-400'>¿Cuándo deseas pagar?</p>
                              <div className='flex items-center gap-4'>
                                ...
                                <span className='flex items-center gap-2 text-xs text-amber-600'>
                                  ...
                                  Disponible durante el periodo de protección de 24 h
                                </span>
                              </div>
                            </div>
