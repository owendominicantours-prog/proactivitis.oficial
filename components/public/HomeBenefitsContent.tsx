...
export function HomeBenefitsContent({ locale }: HomeBenefitsContentProps) {
  const t = (key: TranslationKey) => translate(locale, key);

  return (
    <>
      <div className="space-y-4">
        <p className="text-xs uppercase tracking-[0.4em] text-slate-500">{t("home.section.whatWeDo.label")}</p>
        <h2 className="text-3xl font-semibold text-slate-900">{t("home.section.whatWeDo.title")}</h2>
        <p className="text-sm text-slate-500">{t("home.section.whatWeDo.description")}</p>
      </div>
        ...
        {benefits.map((benefit) => (
          ...
          >
            <div className="flex items-center gap-3 text-emerald-600">
              ...
              <h3 className="text-lg font-semibold text-slate-900">{t(benefit.titleKey)}</h3>
            </div>
            <p className="text-sm text-slate-500">{t(benefit.descriptionKey)}</p>
          </article>
        ))}
      </div>
    </>
