...
import { SANTO_DOMINGO_BASE_TOUR, type SantoDomingoVariant } from "@/data/santo-domingo-variants";
...
type SantoDomingoVariantLandingProps = {
  locale: Locale;
  variant: SantoDomingoVariant;
  tour: {
    id: string;
    slug: string;
    title: string;
...
export default function SantoDomingoVariantLanding({
  locale,
  variant,
  tour,
  transferHotels
}: SantoDomingoVariantLandingProps) {
  ...
  const inclusions =
    SANTO_DOMINGO_BASE_TOUR.inclusions[locale] ?? SANTO_DOMINGO_BASE_TOUR.inclusions.es;
  const highlights =
    SANTO_DOMINGO_BASE_TOUR.highlights[locale] ?? SANTO_DOMINGO_BASE_TOUR.highlights.es;
  const landingSlug = `thingtodo/tours/${variant.slug}`;
  const eyebrow =
    locale === "es"
      ? "Excursion Santo Domingo"
      : locale === "fr"
        ? "Excursion Saint Domingue"
        : "Santo Domingo Day Trip";
  const highlightsLabel =
    locale === "es"
      ? "Ruta y puntos clave"
      : locale === "fr"
        ? "Parcours et points cles"
        : "Route highlights";
            ...
            <div className="flex items-end gap-8">
              ...
              <div>
                ...
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">
                  {t("tour.hero.reviewsCount", { count: 1053 })}
                </p>
              </div>
            </div>
