...
import { PARTY_BOAT_BASE_TOUR, type PartyBoatVariant } from "@/data/party-boat-variants";
...
type PartyBoatVariantLandingProps = {
  locale: Locale;
  variant: PartyBoatVariant;
  tour: {
    id: string;
    slug: string;
    title: string;
...
export default function PartyBoatVariantLanding({
  locale,
  variant,
  tour,
  transferHotels
}: PartyBoatVariantLandingProps) {
  ...
  const inclusions = PARTY_BOAT_BASE_TOUR.inclusions[locale] ?? PARTY_BOAT_BASE_TOUR.inclusions.es;
      ...
      <section className="mx-auto max-w-[1240px] px-4 pt-10">
        <div className="grid gap-6 overflow-hidden rounded-[36px] border border-slate-200 bg-white shadow-[0_30px_60px_rgba(0,0,0,0.06)] lg:grid-cols-2">
          <div className="flex flex-col justify-center gap-6 p-6 sm:p-8 lg:p-14">
            <p className="text-xs uppercase tracking-[0.4em] text-slate-500">Party Boat Punta Cana</p>
            ...
            <div className="flex items-end gap-8">
              ...
              <div>
                ...
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">
                  {t("tour.hero.reviewsCount", { count: 1500 })}
                </p>
              </div>
            </div>
          ...
          <TourGalleryCollage images={parseGallery(tour.gallery)} title={variant.titles[locale]} fallbackImage={heroImage} />
        </div>
      </section>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr,0.9fr]">
          <div className="space-y-6">
            <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-lg">
              <p className="text-xs uppercase tracking-[0.4em] text-slate-500">{t("tour.section.overview.label")}</p>
            ...
            <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-lg">
              <p className="text-xs uppercase tracking-[0.4em] text-slate-500">{t("thingtodo.longform.eyebrow")}</p>
            ...
            <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-lg">
              <p className="text-xs uppercase tracking-[0.4em] text-slate-500">{t("thingtodo.longform2.eyebrow")}</p>
              <h2 className="mt-2 text-[20px] font-semibold text-slate-900">
            ...
            <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-lg">
              <p className="text-xs uppercase tracking-[0.4em] text-slate-500">{t("thingtodo.longform3.eyebrow")}</p>
              <h2 className="mt-2 text-[20px] font-semibold text-slate-900">
            ...
            <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-lg">
              <p className="text-xs uppercase tracking-[0.4em] text-slate-500">{t("tour.section.coverage.label")}</p>
          ...
          <aside className="space-y-6" id="booking">
            <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-xl">
              <p className="text-xs uppercase tracking-[0.3em] text-slate-500">{t("tour.booking.panel.label")}</p>
            ...
            <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-lg">
              <p className="text-xs uppercase tracking-[0.4em] text-slate-500">{t("safetyGuide.reco.transfers.label")}</p>
