...
export const TourCatalog = ({ tours }: { tours: AgencyTourSummary[] }) => {
  ...
  const destinations = useMemo(() => Array.from(new Set(tours.map((tour) => tour.location))), [tours]);
  const suppliers = useMemo(() => Array.from(new Set(tours.map((tour) => tour.supplier))), [tours]);
          ...
          return (
            ...
            >
              ...
              <div className="flex min-w-[180px] flex-col gap-1 text-sm text-slate-600">
                ...
                <p>
                  Precio neto agencia:{" "}
                  <span className="font-semibold text-slate-900">${netAgencyPrice.toFixed(0)}</span>
                </p>
                <p>
                  Comision agencia:{" "}
                  <span className="font-semibold text-slate-900">
                    {tour.commissionPercent}% · ${commissionValue.toFixed(0)}
                  </span>
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.3em] text-slate-600">
                <Link
                  ...
                  className="rounded-md border border-slate-200 bg-slate-50 px-3 py-1 hover:border-slate-300"
                >
                  Ver en web
                </Link>
                <Link
                  href={`/tours/${tour.slug}?from=agency#booking`}
                  className="rounded-md border border-sky-200 bg-sky-50 px-3 py-1 text-sky-700 hover:border-sky-300"
                >
