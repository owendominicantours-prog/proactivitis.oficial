...
export const NUEVA_GENERACION_LOCALES: NuevaGeneracionLocale[] = ["es", "en", "fr"];

const NUEVA_GENERACION_LOCALE_PATHS: Record<NuevaGeneracionLocale, { hub: string; tour: string; intent: string }> = {
  es: {
...
export const getNuevaGeneracionHubPath = (locale: NuevaGeneracionLocale = "es") =>
  NUEVA_GENERACION_LOCALE_PATHS[locale].hub;

export const buildNuevaGeneracionTourPath = (slug: string, locale: NuevaGeneracionLocale = "es") =>
  `${NUEVA_GENERACION_LOCALE_PATHS[locale].tour}/${slug}`;
export const buildNuevaGeneracionIntentPath = (intentSlug: string, locale: NuevaGeneracionLocale = "es") =>
...
> = {
  "con-recogida-en-hotel": {
    en: {
      ...
      audience: "travelers who want to avoid taxis, confusing meeting points and manual coordination",
      angle: "highlights organized departure, WhatsApp support and clear confirmation before payment",
      proof: "The decision is centered on pickup point, schedule and passenger details.",
    ...
    fr: {
      ...
      audience: "voyageurs qui veulent éviter les taxis, les points de rencontre confus et la coordination manuelle",
      angle: "met en avant un départ organisé, le support WhatsApp et une confirmation claire avant le paiement",
      proof: "La décision se concentre sur le point de prise en charge, l'horaire et les informations passagers.",
  ...
  "para-familias": {
    en: {
      ...
      audience: "families that need clarity, a manageable pace and safety before booking",
      angle: "presents the tour as an easy option to coordinate for adults, teens and children",
      proof: "The page highlights duration, preparation, inclusions and support during the trip.",
    ...
    fr: {
      ...
      audience: "familles qui veulent de la clarté, un rythme simple et de la sécurité avant de réserver",
      angle: "présente le tour comme une option facile à coordonner pour adultes, adolescents et enfants",
      proof: "La page met en avant la durée, la préparation, les inclusions et le support pendant l'expérience.",
  ...
  "para-parejas": {
    en: {
      ...
      audience: "couples looking for photos, a memorable experience and a simple booking flow",
      angle: "turns the tour into a romantic or special plan without logistical friction",
    ...
    fr: {
      ...
      audience: "couples qui cherchent des photos, une expérience mémorable et une réservation simple",
      angle: "transforme le tour en plan romantique ou spécial sans friction logistique",
      proof: "Les moments photo, le confort et la valeur de l'expérience sont mis en avant.",
  ...
  "privado-vip": {
    en: {
      ...
      audience: "customers who prefer privacy, flexibility and more personalized attention",
      angle: "positions the tour as a premium option with clear coordination and human support",
      proof: "Support, schedule, provider and operating details are reinforced before payment.",
    ...
    fr: {
      ...
      audience: "clients qui préfèrent intimité, flexibilité et attention plus personnalisée",
      angle: "positionne le tour comme une option premium avec coordination claire et support humain",
      proof: "Le support, l'horaire, le fournisseur et les détails d'opération sont renforcés avant le paiement.",
  ...
  "ultimo-minuto": {
    en: {
      ...
      angle: "emphasizes quick decisions, spaces, availability and direct confirmation",
      proof: "The page reduces doubt with price, schedule, inclusions and immediate booking.",
    ...
    fr: {
      ...
      angle: "met l'accent sur la décision rapide, les places, la disponibilité et la confirmation directe",
      proof: "La page réduit les doutes avec prix, horaire, inclusions et réservation immédiate.",
  ...
  "mejor-precio": {
    en: {
      ...
      proof: "Inclusions, duration, photos and conditions are shown before booking.",
    ...
    fr: {
      ...
      angle: "présente le prix comme une décision complète, pas seulement un chiffre bas",
      proof: "Les inclusions, la durée, les photos et les conditions sont affichées avant de réserver.",
  ...
  "top-rated": {
    en: {
      ...
      angle: "organizes the information so the customer understands why this tour deserves priority",
      proof: "The experience combines real photos, itinerary, provider, FAQs and direct booking.",
    ...
    fr: {
      ...
      angle: "organise l'information pour comprendre pourquoi ce tour mérite la priorité",
      proof: "L'expérience combine photos réelles, itinéraire, fournisseur, FAQ et réservation directe.",
      cta: "Choisir ce tour"
    }
  },
  "comparativa-excursiones": {
    en: {
      ...
      proof: "Includes related tours, differences, operating data and a direct booking path.",
    ...
    fr: {
      ...
      angle: "aide à comparer les alternatives liées avant de choisir une expérience",
      proof: "Inclut des tours liés, différences, données opérationnelles et chemin de réservation directe.",
...
export const getNuevaGeneracionIntents = (locale: NuevaGeneracionLocale = "es") =>
  NUEVA_GENERACION_INTENTS.map((intent) => localizeNuevaGeneracionIntent(intent, locale));

export const getNuevaGeneracionIntent = (slug: string, locale: NuevaGeneracionLocale = "es") => {
  if (locale === "es") return NUEVA_GENERACION_INTENTS.find((intent) => intent.slug === slug) ?? null;
  const match = NUEVA_GENERACION_INTENTS.find((intent) => {
    const localizedSlug = NUEVA_GENERACION_INTENT_LOCALE_COPY[intent.slug]?.[locale]?.slug;
...
const getTourTranslation = (tour: { translations?: Array<Record<string, unknown>> }, locale: NuevaGeneracionLocale) =>
...
export const getNuevaGeneracionTours = async (locale: NuevaGeneracionLocale = "es") =>
...
export const getNuevaGeneracionTourBySlug = async (slug: string, locale: NuevaGeneracionLocale = "es") =>
...
export const getNuevaGeneracionDisplayTitle = (
  tour: { slug: string; title: string; translations?: Array<Record<string, unknown>> },
  locale: NuevaGeneracionLocale = "es"
) => {
  if (tour.slug === "sunset-catamaran-snorkel") {
    if (locale === "en") return "Sunset Catamaran with Snorkeling in Punta Cana";
    if (locale === "fr") return "Catamaran au coucher du soleil avec snorkeling à Punta Cana";
    return "Catamarán al Atardecer con Snorkel en Punta Cana";
  }
  const translatedTitle = getTourTranslation(tour, locale)?.title;
  if (typeof translatedTitle === "string" && translatedTitle.trim()) return fixTourTextTypos(translatedTitle.trim());
  return tour.title;
};
...
export const getNuevaGeneracionTourDescription = (
  ...
  if (typeof translatedShort === "string" && translatedShort.trim()) return fixTourTextTypos(translatedShort.trim());
  if (typeof translatedDescription === "string" && translatedDescription.trim()) {
    return fixTourTextTypos(translatedDescription.trim());
  }
  return fixTourTextTypos((tour.shortDescription ?? tour.description ?? "").trim());
};
...
export const resolveTourPersona = (tour: {
  ...
  const title = getNuevaGeneracionDisplayTitle({ slug: tour.slug ?? "", title: tour.title, translations: tour.translations }, locale);
  const description = getNuevaGeneracionTourDescription(tour, locale);
  const normalized = normalizeIntentText(
    [title, description, tour.category, tour.location].filter(Boolean).join(" ")
  );
  if (/saona|catamaran|boat|barco|isla|snorkel|playa|beach|parasail|mar|ocean/.test(normalized)) {
    const copy = {
      ...
      en: {
        ...
        promise: "a water experience with clear logistics, photo time and support before departure",
        ...
        urgency: "Beach and marina departures depend on seats and transport windows."
      },
      fr: {
        segment: "plage, mer et photos",
        promise: "une expérience aquatique avec logistique claire, temps photo et support avant le départ",
        hook: "Réservez sans improviser transport, horaires ou point de rencontre.",
        packing: "Maillot de bain, vêtements secs, crème solaire et pochette imperméable.",
        urgency: "Les sorties plage et marina dépendent des places et des fenêtres de transport."
      }
    };
    return copy[locale];
  }
  if (/buggy|atv|zip|horse|caballo|safari|aventura|adventure|off-road|offroad/.test(normalized)) {
    const copy = {
      ...
      en: {
        ...
        promise: "an active experience with local stops and advance coordination to avoid delays",
        hook: "Book with organized departure and clear details before reaching the activity point.",
        packing: "Closed shoes, clothes that can get dirty, sunglasses and sunscreen.",
        urgency: "Adventure groups depend on briefing and route schedules."
      },
      fr: {
        segment: "aventure et route active",
        promise: "une expérience active avec arrêts locaux et coordination en avance pour éviter les retards",
        hook: "Réservez avec départ organisé et détails clairs avant d'arriver au point d'activité.",
        packing: "Chaussures fermées, vêtements pouvant se salir, lunettes et crème solaire.",
        urgency: "Les groupes d'aventure dépendent du briefing et des horaires de route."
      }
    };
    return copy[locale];
  }
  if (/santo domingo|historia|cultura|cultural|city|ciudad|museo/.test(normalized)) {
    const copy = {
      ...
      en: {
        ...
        promise: "a guided route with protected timing to visit important places without rushing",
        ...
        urgency: "Cultural routes work best with early departure and confirmed seats."
      },
      fr: {
        segment: "histoire et culture",
        promise: "un parcours guidé avec temps protégé pour visiter les lieux importants sans courir",
        hook: "Idéal si vous voulez du contexte local, transport organisé et visite structurée.",
        packing: "Vêtements légers, chaussures confortables, pièce d'identité et bouteille d'eau.",
        urgency: "Les routes culturelles fonctionnent mieux avec départ tôt et places confirmées."
      }
    };
    return copy[locale];
  }
  if (/samana|ballena|whale|cascada|waterfall|montana|nature|natural|rio/.test(normalized)) {
    const copy = {
      ...
      en: {
        ...
        promise: "a long outing with coordinated connections and support during the route",
        hook: "Avoid losing time coordinating roads, boarding or meeting points separately.",
        packing: "Comfortable shoes, light change of clothes, sunscreen and cash for extras.",
        urgency: "Long excursions depend on weather, road timing and boarding capacity."
      },
      fr: {
        segment: "nature et journée complète",
        promise: "une longue sortie avec connexions coordonnées et support pendant le parcours",
        hook: "Évitez de perdre du temps à coordonner route, embarquement ou points de rencontre séparément.",
        packing: "Chaussures confortables, change léger, crème solaire et espèces pour extras.",
        urgency: "Les longues excursions dépendent de la météo, de la route et des places d'embarquement."
      }
    };
    return copy[locale];
  }
  const fallback = {
    ...
    en: {
      ...
      promise: "a tour with clear price, confirmation and human support for a smooth booking",
      hook: "An organized experience that puts the important information in one place.",
      packing: "Comfortable clothes, sunscreen, charged phone and booking confirmation.",
      urgency: "The best times usually sell first in high season."
    },
    fr: {
      segment: "expérience guidée",
      promise: "un tour avec prix clair, confirmation et support humain pour réserver sans friction",
      hook: "Une expérience organisée avec les informations importantes au même endroit.",
      packing: "Vêtements confortables, crème solaire, téléphone chargé et confirmation de réservation.",
      urgency: "Les meilleurs horaires partent souvent en premier en haute saison."
    }
  };
...
export const buildTourFacts = (
  ...
  const includesFromString = tour.includes
    ...
    : [];
  const includesList = parseTextList(translation?.includesList ?? tour.includesList);
  const notIncluded = parseTextList(translation?.notIncludedList ?? tour.notIncludedList);
...
export const buildNuevaGeneracionConversionCopy = ({
  ...
  const displayTitle = getNuevaGeneracionDisplayTitle(tour, locale);
  const normalized = normalizeIntentText(
    [displayTitle, getNuevaGeneracionTourDescription(tour, locale), tour.category, tour.location, facts.area].filter(Boolean).join(" ")
  );
  ...
  const localizedConversion = {
    es: {
      ...
      adventure: "Vive una ruta activa con briefing, paradas claras y coordinación previa para llegar listo a la experiencia.",
      ...
      defaultExclusions: isParasailing
        ...
        : isWater
          ...
          : ["Gastos personales y propinas", "Servicios no indicados como incluidos", "Cambios fuera del horario confirmado"],
      cancellationTitle: "Cancelación gratuita hasta 24h antes",
      cancellationBody: "Puedes cancelar sin cargo hasta 24 horas antes de la salida confirmada.",
      weatherTitle: isParasailing ? "Salida según viento y seguridad" : "Clima y operación claros",
      ...
      confirmationBody: "Validamos pasajeros, horario, hotel o punto de encuentro para que llegues con la información importante lista.",
      trustBadges: ["Cancelación gratis 24h", isParasailing ? "Operación según viento" : "Precio claro en USD", facts.pickup ? "Recogida coordinada" : "Soporte local"],
    ...
    en: {
      ...
      adventure: "Enjoy an active route with briefing, clear stops and advance coordination before the experience.",
      generic: `${persona.promise}. Book with visible price, clear schedule and human support before departure.`,
      defaultExclusions: isParasailing
        ? ["Professional photos and videos, when offered by the operator, are paid separately", "Tips and personal expenses", "Transfers outside the confirmed zone, if applicable"]
        : isWater
          ? ["Photos, videos or premium drinks not listed as included", "Tips and personal expenses", "Services outside the confirmed itinerary"]
          : ["Personal expenses and tips", "Services not listed as included", "Changes outside the confirmed schedule"],
      cancellationTitle: "Free cancellation up to 24h before",
      cancellationBody: "You can cancel free of charge up to 24 hours before the confirmed departure.",
      weatherTitle: isParasailing ? "Departure depends on wind and safety" : "Clear weather and operating policy",
      weatherBody: isParasailing
      ...
      confirmationBody: "We validate passengers, schedule, hotel or meeting point so you arrive with the important information ready.",
      trustBadges: ["Free cancellation 24h", isParasailing ? "Wind-based operation" : "Clear USD price", facts.pickup ? "Pickup coordinated" : "Local support"],
      bookingPrompt: isParasailing
    ...
    fr: {
      ...
      adventure: "Vivez une route active avec briefing, arrêts clairs et coordination avant l'expérience.",
      generic: `${persona.promise}. Réservez avec prix visible, horaire clair et support humain avant le départ.`,
      defaultExclusions: isParasailing
        ? ["Photos et vidéos professionnelles, si proposées par l'opérateur, payées séparément", "Pourboires et dépenses personnelles", "Transferts hors zone confirmée, si applicable"]
        : isWater
          ? ["Photos, vidéos ou boissons premium non indiquées comme incluses", "Pourboires et dépenses personnelles", "Services hors itinéraire confirmé"]
          : ["Dépenses personnelles et pourboires", "Services non indiqués comme inclus", "Changements hors horaire confirmé"],
      cancellationTitle: "Annulation gratuite jusqu'à 24h avant",
      cancellationBody: "Vous pouvez annuler sans frais jusqu'à 24 heures avant le départ confirmé.",
      weatherTitle: isParasailing ? "Départ selon vent et sécurité" : "Météo et opération claires",
      weatherBody: isParasailing
      ...
      confirmationBody: "Nous validons passagers, horaire, hôtel ou point de rencontre pour arriver avec les informations importantes.",
      trustBadges: ["Annulation gratuite 24h", isParasailing ? "Opération selon vent" : "Prix clair en USD", facts.pickup ? "Pickup coordonné" : "Support local"],
      bookingPrompt: isParasailing
  ...
  const confidence = [
    {
      ...
      body: tour.cancellationPolicy?.trim() || localizedConversion.cancellationBody
    },
    {
  ...
  return {
    heroSubtitle,
    exclusions: uniqueTextItems([...(facts.notIncluded ?? []), ...localizedConversion.defaultExclusions]).slice(0, 6),
    confidence,
...
export const buildNuevaGeneracionSchema = ({
  ...
  const faq = buildNuevaGeneracionFaq(displayTitle, facts.area, persona, intent, locale);
  ...
  return {
    ...
    "@graph": [
      ...
      },
      {
        ...
        name: locale === "en" ? "Alternative tours inside Proactivitis" : locale === "fr" ? "Tours alternatifs dans Proactivitis" : "Tours alternativos dentro de Proactivitis",
        itemListElement: relatedTours.map((related, index) => ({
          "@type": "ListItem",
          position: index + 1,
          name: getNuevaGeneracionDisplayTitle(related, locale),
      ...
      },
      {
        ...
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "Tours", item: `${NUEVA_GENERACION_BASE_URL}${locale === "es" ? "/tours" : `/${locale}/tours`}` },
          { "@type": "ListItem", position: 2, name: locale === "en" ? "Punta Cana excursions" : locale === "fr" ? "Excursions Punta Cana" : "Excursiones Punta Cana", item: `${NUEVA_GENERACION_BASE_URL}${getNuevaGeneracionHubPath(locale)}` },
...
export const buildNuevaGeneracionFaq = (
  ...
  const faqCopy = {
    es: {
      why: intent ? `Por que reservar ${title} como ${intent.keyword}?` : `Por que reservar ${title} con Proactivitis?`,
      whyAnswer: intent
      ...
      weatherAnswer: "Si el clima impide operar de forma segura, coordinamos reprogramacion o la solucion que aplique segun la politica del tour y del operador confirmado.",
      pickup: "La recogida en hotel esta incluida?",
      pickupAnswer: "Cuando el tour incluye recogida, se confirma el hotel, punto de encuentro y horario antes de la salida. Si una zona requiere ajuste, se informa antes de completar la reserva.",
      hiddenFees: "Hay cargos ocultos?",
      hiddenFeesAnswer: "La pagina separa lo incluido y lo no incluido para que puedas ver precio, extras, propinas o servicios opcionales antes de reservar.",
      advance: "Conviene reservar con anticipacion?",
      weight: "Cual es el limite de peso para parasailing?",
      weightAnswer: "El limite exacto depende del operador, el viento y las condiciones del dia. Se confirma antes de operar para mantener la salida segura.",
      tandem: "Pueden volar dos personas juntas en tandem?",
      tandemAnswer: "Muchas salidas permiten vuelo tandem, pero depende del peso combinado, viento y criterio del capitan. La confirmacion se realiza antes de la actividad."
    },
    en: {
      why: intent ? `Why book ${title} as a ${intent.keyword}?` : `Why book ${title} with Proactivitis?`,
      whyAnswer: intent
      ...
      weatherAnswer: "If weather prevents safe operation, we coordinate rescheduling or the applicable solution under the tour and confirmed operator policy.",
      pickup: "Is hotel pickup included?",
      pickupAnswer: "When pickup is included, hotel, meeting point and schedule are confirmed before departure. If a zone requires adjustment, it is shown before completing the booking.",
      hiddenFees: "Are there hidden fees?",
      hiddenFeesAnswer: "The page separates included and not included items so you can see price, extras, tips or optional services before booking.",
      advance: "Should I book in advance?",
      weight: "What is the weight limit for parasailing?",
      weightAnswer: "The exact limit depends on the operator, wind and daily conditions. It is confirmed before operation to keep the departure safe.",
      tandem: "Can two people fly together in tandem?",
      tandemAnswer: "Many departures allow tandem flight, but it depends on combined weight, wind and the captain's decision. Confirmation happens before the activity."
    },
    fr: {
      why: intent ? `Pourquoi réserver ${title} comme ${intent.keyword} ?` : `Pourquoi réserver ${title} avec Proactivitis ?`,
      whyAnswer: intent
      ...
      weather: "Que se passe-t-il s'il pleut ou si la météo empêche l'opération ?",
      weatherAnswer: "Si la météo empêche une opération sûre, nous coordonnons reprogrammation ou solution applicable selon la politique du tour et de l'opérateur confirmé.",
      pickup: "La prise en charge à l'hôtel est-elle incluse ?",
      pickupAnswer: "Quand la prise en charge est incluse, l'hôtel, le point de rencontre et l'horaire sont confirmés avant le départ. Si une zone demande un ajustement, c'est indiqué avant de terminer la réservation.",
      hiddenFees: "Y a-t-il des frais cachés ?",
      hiddenFeesAnswer: "La page sépare les éléments inclus et non inclus pour voir le prix, les extras, pourboires ou services optionnels avant de réserver.",
      advance: "Faut-il réserver à l'avance ?",
      weight: "Quelle est la limite de poids pour le parasailing ?",
      weightAnswer: "La limite exacte dépend de l'opérateur, du vent et des conditions du jour. Elle est confirmée avant l'opération pour garder la sortie sûre.",
      tandem: "Deux personnes peuvent-elles voler ensemble en tandem ?",
      tandemAnswer: "Beaucoup de sorties permettent le vol tandem, mais cela dépend du poids combiné, du vent et de la décision du capitaine. La confirmation se fait avant l'activité."
    }
  }[locale];
...
export const buildIntentLandingCopy = ({
  ...
  decision: [
    {
      title: locale === "en" ? "Who it is for" : locale === "fr" ? "Pour qui" : "Para quien es",
      body:
        locale === "en"
    ...
    {
      title: locale === "en" ? "Before booking" : locale === "fr" ? "Avant de réserver" : "Antes de reservar",
      body: intent.proof
    },
    {
      title: locale === "en" ? "Booking tip" : locale === "fr" ? "Conseil de réservation" : "Consejo de reserva",
