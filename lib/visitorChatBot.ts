...
const makeTourLine = (tour: { slug: string; title: string; price: number; duration: string | null }) =>
  `- ${tour.title} (desde USD ${Math.round(tour.price)}, ${tour.duration ?? "duracion variable"}) https://proactivitis.com/tours/${tour.slug}`;
...
export async function generateVisitorChatReply({ message, pagePath }: SalesReplyInput): Promise<string> {
  ...
  const keyword = text.split(/\s+/).find((word) => word.length >= 4) ?? "";

  const [tours, hotels, transfers] = await Promise.all([
    prisma.tour.findMany({
      where: {
        OR: keyword
          ? [{ title: { contains: keyword, mode: "insensitive" } }, { slug: { contains: keyword, mode: "insensitive" } }]
          : undefined
      },
