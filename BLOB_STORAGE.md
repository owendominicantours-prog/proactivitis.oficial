...
2. The upload API (`POST /api/upload`) validates the file, optimizes it with `sharp`, stores a local copy for debug, and pushes it to Vercel Blob using the token. The response includes `url` (public image URL) and `blobUrl`.
